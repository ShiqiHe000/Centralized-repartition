...
USE MPI
USE INPUT, ONLY: MESHFILE, SPLIT_MAX_NUM
USE SIZE 
...
SUBROUTINE READ_MESH(MESHFILE)
    ...
    INTEGER, ALLOCATABLE, DIMENSION(:) :: DISPLS, SENDCOUNTS
    ...
    IF (RANK ==0) THEN
        ...
        ! CALCULATE THE NELMAX------------------------------------------
        NELMAX = NEL_TOTAL*4**(SPLIT_MAX_NUM)
        !---------------------------------------------------------------
        ...
        ! ALLOCATE ----------------------------------------------------------
        ALLOCATE(NEL_GLOBAL(NUM_PROC))         ! ELEMENT NUMBER IN EACH PROC
    ...
    CALL MPI_BCAST(NELMAX, 1, MPI_INT, 0, MPI_COMM_WORLD, IERROR)
    CALL MPI_SCATTER(NEL_GLOBAL, 1, MPI_INT, NEL_LOCAL, 1, MPI_INT, 0, MPI_COMM_WORLD, IERROR)
    !-------------------------------------------------------------------
    
    ! SCATTER COORDINATE------------------------------------------------
    ALLOCATE(X_LOCAL(4, NEL_LOCAL))
    ALLOCATE(Y_LOCAL(4, NEL_LOCAL))
    ...
    IF (RANK == 0) THEN
        ...
        DO IC=2, NUM_PROC
            DISPLS(IC) = DISPLS(IC-1)+NEL_GLOBAL(IC-1)*4
        ENDDO
        
        SENDCOUNTS=NEL_GLOBAL*4
    ...
    CALL MPI_SCATTERV(X_GLOBAL, SENDCOUNTS, DISPLS, MPI_DOUBLE_PRECISION, &
                        X_LOCAL, NEL_LOCAL*4, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, IERROR)
    CALL MPI_SCATTERV(Y_GLOBAL, SENDCOUNTS, DISPLS, MPI_DOUBLE_PRECISION, &
                        Y_LOCAL, NEL_LOCAL*4, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, IERROR)
    !-------------------------------------------------------------------
