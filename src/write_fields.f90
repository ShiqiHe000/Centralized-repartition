...
USE MPI
USE SIZE
USE INPUT, ONLY: START_COARSEN
USE LOAD_COAREN, ONLY: H_COARSEN_FLAG, GSPLIT_TRACKING 
...
!    PRINT *, "NEL_TOTAL", NEL_TOTAL
    ...
    CALL MPI_GATHERV(SPLIT_TRACKING, NEL_LOCAL, MPI_DOUBLE_PRECISION, GSPLIT_TRACKING, &
            RECVCOUNT/4, DISPLS2, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, IERROR)
    !-------------------------------------------------------------------
    
!    IF(RANK == 0) THEN
!        PRINT *, "GSPLIT_TRACKING", GSPLIT_TRACKING
!    ENDIF
    ...
    IF (RANK == 0) THEN ! ONLY PROCESSOR ONE WRITE
        ...
        WRITE(2,FMT='(''"GPOLY_ODER"'','','')', advance='no')
        WRITE(2,FMT='(''"GSPLIT_TRACKING"'','','')')
...
!                                GH_DEPTH(IEL), GPOLY_ORDER(IEL), GSPLIT_TRACKING(IEL)
!            ENDDO
        ...
        ENDDO
10 FORMAT(F10.5, 2X, F10.5, 2X, I2, 2X, I2, 2X, I2, 2X, F11.8)
