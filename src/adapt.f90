...
REAL(KIND=8), ALLOCATABLE, DIMENSION(:) :: SPLIT_TRACKING    ! TRACKING THE SPLITTING ELEMENT WITH ITS NEIGHBOUR
...
SUBROUTINE HADAPT !-----------------------------------------------------
    ...
    ! H-REFINEMENT
    ! 30% REFINE
    ! REFINEMENT DEPTH <= SPLIT_MAX_NUM 
    ! (EACH PROCESSOR <= NEL_TOTAL*4^(SPLIT_MAX_NUM) ELEMENT)
    !-------------------------------------------------------------------
    ...
    REAL(KIND=8), ALLOCATABLE, DIMENSION(:) :: SPLIT_TRACKING_NEW    ! TRACKING THE SPLITTING ELEMENT WITH ITS NEIGHBOUR
    ...
    ALLOCATE(XX(3), YY(3))
    ALLOCATE(SPLIT_TRACKING_NEW(NELMAX))
    !-------------------------------------------------------------------
    
    ! INITIALIZATION----------------------------------------------------
    NEL_LOCAL_NEW=1
    SPLIT_TRACKING_NEW=0
    !-------------------------------------------------------------------
    ...
    DO IEL=1, NEL_LOCAL ! LOOP THROUGH ALL ELEMENTS
        ...
        ! 30% ELEMENT H REFINE
        IF ((RAND_NUM <= 0.3) .AND. (H_DEPTH(IEL) < SPLIT_MAX_NUM+1)) THEN ! H-REFINEMENT------------------------
            XX(1)=X_LOCAL(1, IEL)
            ...
            ! H REFINEMENT GEENERATES 4 NEW ELEMENTS--------------------
            DO I=1,4
                IF ((I == 1) .OR. (I == 3)) THEN
                ...
                SPLIT_TRACKING_NEW(NEL_LOCAL_NEW) = REAL(I, KIND=8) + SPLIT_TRACKING(IEL)/10.0D0  ! RECORD ELEMENT SPLIT LOCATION
        ...
        ELSE !------NO REFINEMENT, STORE THE ELEMENT COORDINATE IN THE NEW GLOBAL ARRAY
            ...
            SPLIT_TRACKING_NEW(NEL_LOCAL_NEW) = SPLIT_TRACKING(IEL)
    ...
    ! UPDATE THE SPILT_TRACKING NUM-------------------------------------
    DEALLOCATE(SPLIT_TRACKING)
    ALLOCATE(SPLIT_TRACKING(NEL_LOCAL))
    SPLIT_TRACKING(:) = SPLIT_TRACKING_NEW(1:NEL_LOCAL)
    !-------------------------------------------------------------------
